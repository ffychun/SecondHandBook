...
<table>
    ...
    <tr>
        ...
        <td><button onclick="bookDetail(<%= cartItem.getBookId() %>)">商品详情</button></td>
        <td>已售罄</td>
    ...
    <tr>
        ...
        <td><img src="<%= cartItem.getPath() %>"></td>
        <td><button onclick="bookDetail(<%= cartItem.getBookId() %>)">商品详情</button></td>
        <td><button onclick="purchaseBook(<%= cartItem.getBookId() %>,<%= cartItem.getAmount() %>)">购买商品</button></td>
        <td><button onclick="deleteProduct(<%= cartItem.getBookId() %>)">删除</button></td>
    </tr>
    <% }
    }%>
...
<script>
    ...
    function purchaseBook(bookID,quantity){
        window.location.href = "purchaseFromCart.jsp?bookId=" + bookID+"&quantity="+quantity;
    }
